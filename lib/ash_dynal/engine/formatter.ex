defmodule AshDynal.Engine.Formatter do
  @moduledoc """
  ...
  @doc "Format raw records into an `AshDynal.Result`."
  ...
  defp frequency(%{column: column, group_by: []}, records) do
    counts = Enum.frequencies_by(records, fn row -> Map.get(row, column) end)
    labels = counts |> Map.keys() |> Enum.map(&to_label/1) |> Enum.sort()
    data = Enum.map(labels, fn label -> Map.get(counts, label_to_key(counts, label), 0) end)
  ...
  defp aggregate(%{column: column, function: function, group_by: group_by}, records) do
    grouped = Enum.group_by(records, fn row -> group_key(row, group_by) end)
    ...
    data =
      Enum.map(labels, fn label ->
        key = label_to_key(grouped, label)
        rows = Map.get(grouped, key, [])
  ...
  defp time_bucket(
    ...
    enriched =
      Enum.map(records, fn row ->
        ...
        Map.put(row, :__dynal_bucket__, AshDynal.Engine.TimeBucket.label(ts, bucket))
      end)
  ...
  defp pivot(records, label_field, group_by, aggregate_fn, _series_base_name, type) do
    ...
    labels =
      records
      |> Enum.map(fn row -> Map.get(row, label_field) end)
      |> Enum.uniq()
      |> Enum.map(&to_label/1)
      |> Enum.sort()

    series =
      Enum.map(grouped, fn {group_combo, rows} ->
        by_label = Enum.group_by(rows, fn row -> Map.get(row, label_field) end)

        data =
          Enum.map(labels, fn label ->
            key = label_to_key(by_label, label)
            aggregate_fn.(Map.get(by_label, key, []))
          end)

        %{name: group_name(group_combo), data: data}
      end)
  ...
  # `Enum.frequencies_by` / `Enum.group_by` key on the raw value, but labels are
  # stringified. Map a label back to the original key so we can look values up.
  defp label_to_key(map, label) when is_map(map) do
    Enum.find_value(map, nil, fn {key, _} ->
      if to_label(key) == label, do: key
    end)
  end
