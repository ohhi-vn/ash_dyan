defmodule AshDynal.Dsl.Verifiers.ValidateAnalyzableFields do
  @moduledoc """
  ...
  alias AshDynal.Dsl.AnalyzableField
  ...
  defp validate_field(resource, %AnalyzableField{type: :frequency, name: name} = field) do
  ...
  defp validate_field(resource, %AnalyzableField{type: :aggregate, name: name, functions: []}) do
    ["dynal: analyzable_field :#{name} (aggregate) must declare at least one function"]
  end

  defp validate_field(resource, %AnalyzableField{type: :aggregate, name: name} = field) do
    if attribute_exists?(resource, name) do
      []
    else
  ...
  defp validate_field(resource, %AnalyzableField{type: :time_bucket, name: name, buckets: []}) do
  ...
  defp validate_field(resource, %AnalyzableField{type: :percentile, name: name, percentiles: []}) do
    ["dynal: analyzable_field :#{name} (percentile) must declare at least one percentile"]
  end

  defp validate_field(resource, %AnalyzableField{type: :percentile, name: name} = field) do
    if attribute_exists?(resource, name) do
      []
    else
